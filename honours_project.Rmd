...
fit_models <- function(model, grid, data){
  ...
  perf <- perf %>% mutate(roc_auc = roc_auc$.estimate)
  perf <- perf %>% mutate(sens =  sens$.estimate)
  perf <- perf %>% mutate(spec = spec$.estimate)
...
training_results <- foreach(x=1:10) %do% {
  model_results <- foreach(y=1:3) %do% {
    fit_models(grid_results[[x]]$wflow_id[[y]], grid_results[[x]]$result[[y]], folds$splits[[x]]) 
  }
}
#bind models by row
training_result <- foreach(x=1:10) %do% {
  model_bind <- foreach(y=1:1) %do% {
    rbind(training_results[[x]][[y]], training_results[[x]][[y+1]], training_results[[x]][[y+2]])
  }
}

#filter model across all folds
temp <- map_dfr(training_result, ~ .x[[1]])
filter(temp, Model=="decision_tree")
```
