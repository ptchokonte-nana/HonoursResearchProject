...
folds <- nested_cv(train_data,outside = vfold_cv(repeats = 1),inside = bootstraps(times = 5))
# recipe ------------------------------------------------------------------
...
for (i in 1:nrow(folds)) {
    result <- all_workflow %>% workflow_map(seed = 10503,
                                  resamples = folds$inner_resamples[[i]],
                                  control = grid_ctrl)
    grid_results <- append(grid_results, result)
}
...
rf_fit <- fit_model("random_Forest", grid_results$result[[1]], train_data)
dt_fit <- fit_model("decision_tree", grid_results$result[[2]], train_data)
lr_fit <- fit_model("logistic_regression", grid_results$result[[3]], train_data)
```
