...
lda <- parameters(penalty(), regularization_method())
en <- parameters(penalty(), mixture())
#svm <- parameters(cost(), margin())
param <- list(rf, c50, lda, en)
wf <- all_workflow[-5,] %>% 
  option_add(param_info = param[1], id = "random_Forest") %>%
  option_add(param_info = param[2], id = "C50") %>% 
  option_add(param_info = param[3], id = "LDA") %>%  
  option_add(param_info = param[4], id = "elastic_net")
```
