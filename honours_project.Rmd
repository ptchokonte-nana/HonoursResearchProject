...
grid_results <- all_workflow %>% workflow_map(seed = 10503,
                                              resamples = folds,
                                              control = grid_ctrl)
...
rf_pred <- fit_model("random_Forest", grid_results, train_data)
rf_pred %>% predict(train_data,type = "prob") %>% 
  bind_cols(train_data %>% select(TB_status)) %>%
  roc_auc(TB_status, .pred_Negative)
rf_pred %>% predict(train_data,type = "prob") %>% 
  bind_cols(train_data %>% select(TB_status)) %>%
  roc_curve(TB_status, .pred_Negative) %>% autoplot()

dt_pred <- fit_model("decision_tree", grid_results, train_data)
dt_pred %>% predict(train_data,type = "prob") %>% 
  bind_cols(train_data %>% select(TB_status)) %>%
  roc_auc(TB_status, .pred_Negative)

lr_pred <- fit_model("logistic_regression", grid_results, train_data)
lr_pred %>% predict(train_data,type = "prob") %>% 
  bind_cols(train_data %>% select(TB_status)) %>%
  roc_auc(TB_status, .pred_Negative)
```
