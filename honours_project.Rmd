...
pacman::p_load(pacman, tidyverse, janitor, tidymodels, knitr, missForest, splitstackshape)
...
# folds <- nested_cv(train_data,outside = vfold_cv(repeats = 5),inside = bootstraps(times = 15))
folds <- nested_cv(train_data,outside = vfold_cv(v=10, repeats = 1, strata=TB_status),inside = vfold_cv(v=10, repeats = 5, strata = TB_status))
# recipe ------------------------------------------------------------------
...
grid_results <- all_workflow %>% workflow_map(seed = 10503,resamples = folds,control = grid_ctrl)
best_result <- grid_results %>% rank_results(rank_metric = "accuracy",select_best = TRUE)  %>% 
  filter(.metric == "accuracy") %>%  
  select(wflow_id, .config, accuracy = mean) 
...
fit_model <- function(model,grid,data){
  ...
  reg_metric <- metric_set(roc_auc)
  
  perf <- model_fit %>%
    predict(data, type = "prob") %>%
    bind_cols(data %>% select(TB_status)) %>%
    reg_metric(TB_status, .pred_Negative) %>%
    mutate(object = model, data = "train")
  
  return(perf)
}

rf_fit <- fit_model("random_Forest", grid_results, train_data)


dt_fit <- fit_model("decision_tree", grid_results, train_data)


lr_fit <- fit_model("logistic_regression", grid_results, train_data)
```
