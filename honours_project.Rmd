...
lda <- parameters(regularization_method(c('diagonal', 'min_distance', 'shrink_cov', 'shrink_mean')))
knn <- parameters(neighbors(), weight_func(c('rectangular', 'triangular', 'epanechnikov', 'biweight', 'triweight', 'cos', 'inv', 'gaussian', 'rank', 'optimal')), dist_power())
...
bayesian_grid_results <- foreach(i=1:nrow(folds)) %do% {
...
roc_curve <- foreach(x=1:6) %do% {
  model_pred <- foreach(y=1:10) %do% {
    prediction_results.copy[[y]][[x]] %>% unnest(cols = c(Resamples)) %>% rename(Resamples=id)
  }
...
pROC_obj <- foreach(x=1:6) %do% {
  ...
  par(pty = "s")
  plot(my_ROC, legacy.axes=TRUE, grid = TRUE, main = workflows$wflow_id[[x]])  
  plot(ci.se(my_ROC))
}
...
#-------------------TRAINING AND TEST DIFFERENCE----------------------------
diff <- foreach(x=1:6) %do% {
    tibble(Model = workflows$wflow_id[[x]]) %>% 
    mutate(roc_auc = training_average$roc_auc[[x]]- test_results$roc_auc[[x]], 
    sens = training_average$sens[[x]]- test_results$sens[[x]],
    spec = training_average$spec[[x]]- test_results$spec[[x]])
} %>% bind_rows()
#-----------------------DELONG TEST------------------------------------
cl <- makeCluster(4, type = "FORK")
registerDoParallel(cl)
...
#p-values
p_vals <- c(0.00712, 0.982, 0.0179, 0.000505, 0.0335,
            0.00568, 0.553, 0.258, 0.375, 
            0.0647, 0.000451, 0.0524, 
            0.108, 0.808, 
            0.01152)
...
diff2 <- foreach(a=1:5) %do% {
  diff2_temp <- foreach(x=(a+1):6) %do% {
    test_diff <- test_results$roc_auc[[a]]-test_results$roc_auc[[x]]
    lst <- append(lst, test_diff)
  } 
}
mat[upper.tri(mat)] <- combn(colNames, 2, function(x) round(lst, 5) ) 

# mat <- round(mat,2)
rownames(mat) = colnames(mat) = colnames(dummy)
mat
long <- reshape2::melt(mat)
long$auc <- c(NA, 0.04227, 0.0008, 0.032440, 0.05788, 0.03048,NA,NA, 0.04307, 0.00983, 0.01561, 0.0118,NA,NA,NA, 0.03324, 0.05868, 0.03127, NA,NA,NA,NA, 0.02544, 0.00197,NA,NA,NA,NA,NA, 0.0274,NA,NA,NA,NA,NA,NA)
long %>%
...
#-------------------------------------
dust(bayesian_training_average) %>% 
  sprinkle(col = 3:5, round = 3) %>%
  sprinkle_align(cols = 3:5) %>%
  sprinkle_border(border = c("top", "bottom"), border_style = "solid") %>%
  kable()
