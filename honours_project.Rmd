...
cl <- makeCluster(8, type = "FORK"); registerDoParallel(cl) 
...
# # grid tuning try2--------------------------------------------------------------
# grid_ctrl <- control_grid(save_pred = TRUE, parallel_over = "everything",save_workflow = TRUE)
# grid_results <- function(object, wf, grid){
#   wf %>% workflow_map(seed=1503, resamples=object, grid=25, control=grid)
# }
# 
# plan(multicore)
# tuning_results <- future_map(folds$inner_resamples, grid_results, wf=all_workflow, grid=grid_ctrl)
# # grid tuning try3--------------------------------------------------------------
# grid_ctrl <- control_grid(save_pred = TRUE, parallel_over = "everything",save_workflow = TRUE)
# grid_results <- function(object, wf, grid){
#   wf %>% workflow_map(seed=1503, resamples=object, grid=25, control=grid)
...
fit_models <- function(model, grid, data){
  best_result <- 
    ...
    summarize(roc_auc = mean(mean, na.rm=TRUE))
  model_fit <- 
  ...
  perf <- pred %>% mutate(roc_auc = roc_auc(TB_status, .pred_Negative))
  perf <- pred %>% mutate(sensitivity = sensitivity(TB_status, .pred_class, estimate = .pred_class))
  perf <- pred %>% mutate(specificity = specificity(TB_status, .pred_class, estimate = .pred_class))
  
  final <- perf %>% mutate(Model = model)
  
  return(final)
}
best_results_folds <- function(object, fold) {
    map2(object$wflow_id, object$result, fit_models, data=fold)
}
plan(multisession, workers=8)
train_results <- future_map2(tunning_results, folds$splits, best_results_folds)
```
